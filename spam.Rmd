...
pacman::p_load(janitor, pins, themis, usemodels, tidymodels, tidyverse)
```
...
2. El modo. El cual puede ser clasficiación o regresión.
...
Definiremos los distintos *kernels* a través de definiciones separadas. Solo
para el caso del kernel lineal definiremos para efectos de prueba un segundo
motor: LiblineaR.  Para el resto de modelos utilizaremos el motor de *kernlab*.
...
significando esto que hay perdida de información.
...
El primer set de métricas será para predicciones de clase.
...
pin_write(board = tablero,
			 ...
			 type  = "rds",
			 title = "all_models",
			 description = "9300 modelos entrenados")
```
...
```{r, paged.print = FALSE}
tune_rank <- tune_res %>%
	rank_results(select_best = TRUE, rank_metric = c("specificity", "kap")) %>% 
...
```{r, all-models, fig.cap='(ref:all-models), 'fig.width=13, fig.asp=0.4}
autoplot(tune_res, select_best = TRUE) + drako
```


En la tabla \@ref(tab:modelos) y en la figura \@ref(fig:all-models) vemos 
`r nrow(tune_rank)` combinaciones de modelos con diferentes motores y algoritmos
de balanceo. 

<p class="comment">
Lo que vemos es que al ordenar primero por especificidad y luego por kappa el
modelo más alto del motor LiblineaR se encuentra en el 9no lugar. 
En vista de que los mejores modelos en términos de especificidad
tienen la capacidad de devolver predicciones de tipo continua (probabilidades),
realizaremos un reajuste, pero esta vez incluiremos métricas de tipo *probs* y
*class*.  Esto lo hacemos de esta manera debido a que no es posible realizar el
*tuning* de hiperparámetros de manera simultánea entre modelos cuando alguno de
los motores (ej. LiblineaR) solo tiene la capacidad de devolver un tipo de
predicción (ej. solo predicciones de clase). Si lo hacemos genera un error ya
que `workflow_map` intenta aplicar todas las métricas a todos los modelos.  Otro
enfoque podría ser ajustar los modelos por separado y luego unirlos.
</p>
...
Realicemos un reajuste, pero esta vez consideremos métricas de probabilidad
también.  Primero seleccionemos los tres mejores modelos
...
Realicemos un reajuste nuevamente solo con estos tres modelos.

```{r, eval=FALSE}
calibrate_tune <- curated_workflow %>% 
	workflow_map(fn       = "tune_grid",
					verbose   = TRUE,
					resamples = spam_folds,
					control   = grid_ctrl,
...
```{r, echo=FALSE}
calibrate_tune <- pin_read(board = tablero, name = "top_res")
```
...
Vemos en la columna `r coloring_font("**result**", "#A24000")` que ya se
encontraron los hiperparámetros óptimos.
...
pin_write(board = tablero,
			 ...
			 type  = "rds",
			 title = "top_3",
			 description = "Un total de 960 modelos entrenados")
```
...
```{r}
nrow(collect_metrics(calibrate_tune, summarize = FALSE))
```
...
ct <- calibrate_tune %>%
	...
	pivot_wider(names_from = .metric, values_from = mean) %>% 
	rename(kappa = kap) %>% 
	relocate(specificity, roc_auc, .after = "rank")
```
...
```{r}
calibrate_tune %>% 
	extract_workflow_set_result(fx) %>% 
	select_best(metric = "roc_auc")
```


<p class="comment">
En el primer *tuning* con todas las combinaciones el modelo con mejor
especificidad fue `r max_spe`, y en el segundo *tuning* fue `r fx`.
</p>
