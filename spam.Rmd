...
```{r}
# el primer nivel es nonspam y el segundo nivel es spam
contrasts(spam$type)
```

Crearemos una función para flexibilizar el gráfico de AUC, aunque no sea
posible generalizar para los datos de prueba.
   ...
   roc_curve(type, .pred_spam, event_level = "second") %>%  
	ggplot(aes(x = 1 - specificity, y = sensitivity, color = modelo)) +
		...
		annotate("text",
					...
					size  = 10) +
		drako + theme(legend.position = c(0.7, 0.1)) +
		labs(title = "AUC sobre datos de prueba")
	
```

En el gráfico \@ref(fig:roc-test) vemos que el AUC del modelo KNN es más alto.
