...
Esto es especialmente útil para casos en los que la subestimación de la
variable objetivo no es aceptable, pero se puede tolerar la sobreestimación.

Supongamos que sobrestimamos la renta de bicicletas, el adminstrador puede
tener más bicicletas disponibles para atender la demanda suponiendo que el
costo de mantener más bicicletas es menor que el costo de no rentarlas. El
problema se daría si la cantidad de bicicletas predichas es menor, en este caso
es probable que el administrador pierda dinero por no tener suficientes
bicicletas para suplir la demanda.
...
```
```{=html}
<!-- correo como spam) son muy indeseables. Si insistimos en cero
falsos positivos en -->
```
```{=html}
<!-- el conjunto de entrenamiento / prueba, el 20-25%
del spam pasó a través del -->
```
<!-- filtro. -->
...
(ref:desbalance) El desafío de trabajar con conjuntos de datos desequilibrados
es que la mayoría de las técnicas de aprendizaje automático ignorarán y, a su
vez, tendrán un rendimiento deficiente en la clase minoritaria, aunque
normalmente lo más importante es el rendimiento en la clase minoritaria.
...
(ref:smot) Realiza un sobre muestreo en el que la clase minoritaria se
sobremuestrea creando ejemplos "sintéticos" en lugar de sobremuestrear con
reemplazo.
...
```{r}
knn_kknn <- nearest_neighbor(neighbors   = tune(),   # este es la cantidad de vecinos k a estimar
									  weight_func = tune()) %>% 
...
```{r}
svm_l_liblinear <- svm_linear() %>%
	set_mode("classification") %>%
	set_engine("LiblineaR")
```

```{r}
svm_l_kernlab <- svm_linear() %>%
	set_mode("classification") %>%
	set_engine("kernlab")
```

```{r}
svm_r_kernlab <- svm_rbf(cost = tune(), rbf_sigma = tune()) %>% 
  set_engine("kernlab") %>% 
  set_mode("classification")
```
...
modelos <- list(knn             = knn_kknn,
					 svm_l_liblinear = svm_l_liblinear,
...
Ahora aplicaremos la función `workflow_map` para aplicar cada modelo a su
receta correspondiente dentro de cada pliego (fold).
...
```{r}
mset_01 <- metric_set(accuracy, kap, specificity)
```
...
```{r, modelos, fig.width=13, fig.asp=0.4}
autoplot(tune_res, select_best = TRUE) + drako
```

<p class="comment">
En la tabla \@ref(tab:modelos) vemos más de 20 combinaciones de modelos con
diferentes motores y algoritmos de balanceo. En vista de que los mejores modelos
en términos de especificidad tienen la capacidad de devolver predicciones de
tipo continua (probabilidades), realizaremos un reajuste, pero esta vez
incluiremos métricas de tipo *probs* y *class*.  Esto lo hacemos de esta manera
debido a que no es posible realizar el *tuning* de hiperparámetros de manera
simultánea entre modelos cuando alguno de los motores (ej. LiblineaR) solo tiene
la capacidad de devolver un tipo de predicción (ej. solo predicciones de clase).
Si lo hacemos genera un error ya que workflow_map intenta aplicar todas las
métricas a todos los modelos.  Otro enfoque podría ser ajustar los modelos
por separado, pero es menos eficiente en cuanto a cantidad de código.
</p>

Primero seleccionemos los tres mejores modelos
...
A partir del workflow original, filtremos estos únicos modelos

```{r}
curated_workflow <- spam_workflow %>% 
	filter(wflow_id %in% top_3)
```

Definamos un set de métricas que incluya una métrica de probabilidades: ROC AUC

```{r}
mset_02 <- metric_set(roc_auc, accuracy, kap, specificity)
```

Para acelerar aun más el proceso, esta vez utilicemos el método de carrera
para la búsqueda en cuadrícula.
...
Realicemos un reajuste nuevamente solo con estos tres modelos y con el método
de carrera.

```{r}
full_results_time <- system.time(
		tune_res2 <- curated_workflow %>% 
...
```{r}
nrow(collect_metrics(tune_res2, summarize = FALSE))
```

```{r, conroc, paged.print = FALSE}
tune_res2 %>%
	rank_results(select_best = TRUE) %>% 
	select(modelo = wflow_id, .metric, mean, rank) %>% 
	pivot_wider(names_from = .metric, values_from = mean) %>% 
...
```{r, segunda-vuelta, fig.width=13, fig.asp=0.4}
autoplot(tune_res2, select_best = TRUE) + drako
```


<p class="comment">

Aunque el modelo `smote_knn` tiene la mejor especificidad, seleccionaremos el
modelo SMV radial con ajuste de desbalance SMOT y tratamiento de atípicos
(`smout_svm_radial`) debido a que el resto de sus métricas son mejores.

</p>

(ref:valoresx) Mejores valores de la función de costo y sigma

```{r, valoresx, fig.cap='(ref:valoresx)', fig.width=11, fig.asp=0.6}
autoplot(tune_res, select_best = TRUE, id = "smout_svm_radial") + 
	labs(title = "SVM con Balanceo SMOT", 
		  subtitle = "Tratamiento de atípicos con algoritmo Spatial Sign") +
	drako +
...
```{r}
tune_res %>% 
   extract_workflow_set_result("smout_svm_radial") %>% 
	select_best(metric = "specificity")
```

En la figura \@ref(fig:valoresx) en el cuadrante de especificidad vemos el
valor óptimo de la función de costo el cual con esta mayor penalización se
reduce la probabilidad de clasificación errónea. También vemos que la función
sigma (o gamma whatever)
