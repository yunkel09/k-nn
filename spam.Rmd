...
(ref:smot) Realiza un sobremuestreo en el que la clase minoritaria se
...
```{r}
knn_recipe_smote_out <- knn_recipe_smote %>% 
...
```{r}
svm_l_spec <- svm_linear(cost = (), )
```



El modelo `svm_rbf()` utiliza una **función no lineal**, específicamente una
función kernel radial, para crear el límite de decisión.
...
```{r}
preprocesadores_knn <- list(smote = knn_recipe_smote, 
									 smout = knn_recipe_smote_out, 
									 submu = knn_recipe_submuestreo,
									 sobre = knn_recipe_sobremuestreo,
									 rosex = knn_recipe_rose)
```
...
```{r}
modelos <- list(knn = knn_spec,
					 # svm_lineal = svm_l_spec)
					 svm_radial = svm_r_spec)
```

```{r}
spam_workflow <- workflow_set(preproc = preprocesadores_knn, 
										models = modelos, 
										cross = TRUE)
```
...
```{r}
mset <- metric_set(accuracy, roc_auc, kap, specificity)
```
