...
así como en la aplicación de coherencia en los nombres de las
funciones y los valores de retorno. 
...
muestreo aleatorio se podría dar el caso que en los datos de entrenamiento los
casos de cantidades de rentas extremas (altas o bajas) no estarían bien
...
* **receta_rf:** Todos los predictores sin transformaciones.
...
Antes de construir la receta, veamos como quedarían los predictores numéricos
con las correciones de asimetrías ya realizadas.
...
bike_transformado <- recipe(cnt ~ ., data = bike_train) %>%
	...
	bind_cols(bike_train %>% select(where(is.double)))
```

(ref:predtr) Corrección de asimetría

```{r, predtr, fig.cap='(ref:predtr)', fig.width=10, fig.asp=0.9}
bike_transformado %>% 
	pivot_longer(cols = temp_tr:windspeed,
						 names_to = "variable",
						 values_to = "valores") %>% 
...
En la figura \@ref(fig:predtr) vemos como el paquete {bestNormalize} seleccionó
para cada predictor la transformación que da mejor resultado.  Ahora que hemos
visto como quedan los predictores numéricos transformados, procedamos a realizar
la transformación en la receta.

Algo muy importante de notar es que para que la receta funcione en los casos en
que se utilice la función `step_best_normalize` es necesario aplicar la función
`prep()` al final.  Aun no tenemos claridad del porque esto es necesario, pero
el no hacerlo genera un error en el *workflow*.

```{r}
receta_rf_cetr <- recipe(cnt ~ ., data = bike_train) %>%
	step_best_normalize(all_predictors(), -all_nominal()) %>% 
	prep()
```

Veamos como se ven los datos con los predictores numéricos ya transformados:

```{r}
receta_rf_cetr %>% prep(bike_train) %>% bake(bike_train)
```


Agregaremos algunos pasos de preprocesamiento adicionales. Con base a la figura
\@ref(fig:desbalance) agregaremos `step_other()` para agrupar valores que
ocurren con poca frecuencia y los pondrá en un categoría denominada "otros".

```{r}
receta_rf_cetrplus <- recipe(cnt ~ ., data = bike_train) %>%
	step_best_normalize(all_predictors(), -all_nominal()) %>% 
	step_other(holiday, weathersit) %>% 
...
```{r}
receta_rf_cetrplus %>% prep(bike_train) %>% bake(bike_train)
```

Vemos que el filtro de `step_nzv` eliminó la variable *holiday*.
...
Debido a que no podremos combinar las recetas de RF con el motor de xgboost,
tendremos que separar los *workflows*, realizar el *tuning* y luego unir para
evaluar las métricas

```{r}
preprocesadores_rf <- list(simple_rfr = receta_rf, 
									centradosf = receta_rf_ce,
									asimetrias = receta_rf_cetr,
									all_transf = receta_rf_cetrplus)
```


```{r}
modelo_rf <- list(rf = rf_spec)
```

Usaremos `cross = TRUE` por defecto para que genere todas las combinaciones de
estos componentes. Es decir, todas las recetas contra todos los modelos.

```{r}
bike_set_rf <- workflow_set(preprocesadores_rf, modelo_rf, cross = TRUE)
```
...
Vemos la combinación de todas los preprocesados y los modelos únicamente para
RF.  Ahora lo mismo para XGBoost

```{r}
preprocesadores_xg <- list(xgb_normal = receta_xgboost)
```

```{r}
modelo_xg <- list(xg = xgboost_spec)
```

```{r}
bike_set_xg <- workflow_set(preprocesadores_xg, modelo_xg, cross = TRUE)
```

```{r}
bike_set <- bike_set_rf %>% 
	bind_rows(bike_set_xg)
```
...
# Reduce de 8.5 min a 1.66 min el tiempo de procesamiento de los modelos
...
El tamaño de la cuadrícula es un poco arbitrario. En este caso decidimos colocar
20, pero también pudimos haber creado una cuadrícula con los valores de MTRY y
n_trees que consideremos. Una razón para no hacer esto es que tendríamos que
realizar por separado el tuning de los parámetros de XGBoost, es decir, crear
una cuadrícula para RF y otra para XGBoost.
...
```{r}
# muy importante detener el cluster y eliminar variables asociadas.
stopCluster(cl)
unregister()
```
...
```{r}
nrow(collect_metrics(tune_res, summarize = FALSE))
```
...
```{r, rmse-train, paged.print = FALSE}
tune_res %>%
	rank_results(select_best = TRUE) %>% 
	filter(.metric == "rmse") %>% 
	select(modelo = wflow_id, .config, rmse = mean, rank) %>% 
	tabla(cap = "Ajuste RMSE con datos de entrenamiento")
```
...
rmse_train <- tune_res %>%
	...
	filter(.metric == "rmse") %>% 
	select(modelo = wflow_id, .config, rmse = mean, rank) %>% 
	filter(modelo == "simple_rfr_rf") %>% 
	pull(rmse)
```


A parte de que vemos de que Random Forest se ajustó mejor que XGBoost, vemos que
prácticamente no hay diferencias significativas en el resultado obtenido con los
preprocesamientos adicionales y el ajuste de RF sin procesar. Así que
seleccionaremos el modelo sin preprocesamiento, el cual obtuvo un RMSE de
`r round(rmse_train, 2)`
...
Seleccionemos el Random Forest simple para ver como quedaron sus parámetros.

(ref:valoresx) Mejores valores de `mtry` y `min_n`

```{r, valoresx, fig.cap='(ref:valoresx)', fig.width=12, fig.asp=0.9}
autoplot(tune_res, select_best = TRUE, id = "simple_rfr_rf") + 
	labs(title = "Random Forest Simple", subtitle = "Sin preprocesamiento") +
	drako
```

<p class="comment">
En la gráfica \@ref(fig:valoresx) podemos ver los diferentes valores de RMSE para
cada combinación de `mtry` y de `ntree` con base al número de remuestreos.
</p>
...
best_results <- tune_res %>% 
   extract_workflow_set_result("simple_rfr_rf") %>% 
...
```{r, include=FALSE}
aa <- best_results$mtry
ab <- best_results$min_n
```

Esto es consistente con lo que vimos en la gráfica. Los mejores parámetros del
modelo funcionan bien con `r aa` para el `mtry` y `r ab` para el `min_n`
...
```{r}
rf_test_results <- tune_res %>% 
   extract_workflow("simple_rfr_rf") %>% 
...
rf_test_results %>%
	collect_metrics() %>% 
	tabla(cap = "Ajuste RMSE con datos de prueba")
```

```{r, echo=FALSE}
rmse_prueba <- rf_test_results %>%
	collect_metrics() %>% 
	filter(.metric == "rmse") %>% 
	pull(.estimate)
```
...
<p class="comment">
En la tabla \@ref(tab:rmse-train) vimos que el RMSE para los datos de
entrenamiento fue de `r round(rmse_train, 2)` y con los datos de prueba
obtuvimos un valor de `r round(rmse_prueba, 2)`, lo cual nos da una diferencia
de `r round(rmse_prueba - rmse_train)` Debido a que esta diferencia no es tan
grande no podríamos decir de que tenemos un sobre-ajuste (overfitting).
</p>
...
<p class="comment">
Vemos que el mejor RMSLE es de `r round(tf, 2)`. `r resaltar("Este valor NO es tan bueno
...
<p class="comment">
El RMSE es de `r round(tt$.estimate[1], 2)` bicicletas rentadas por día. 
</p>
...
rentadas.")`. Partial Least Squares
</p>
