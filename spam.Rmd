...
```{r}
mset <- metric_set(roc_auc, accuracy, kap, specificity)
```
...
de balanceo. La especificidad es la proporción de ceros (nonspam) estimados
como ceros, es decir, que proporción de correos de tipo "no spam" fueron
clasificados correctamente como "no spam".  Esto es relevante dada la
preocupación de que un correo legítimo sea considerado como "spam".

Seleccionemos los dos mejores modelos de cada tipo.  En este caso vamos a
omitir el primer modelo debido a que el coeficiente de Cohen Kappa es muy bajo,
...
```{r}
roc_kear <- function(top_models, wflow, evto_pred, verdad) {
	...
	# calcular el area bajo la curva para cada modelo	
	auc <- .datos %>%
		group_by(modelo) %>%
	...
	.datos %>%
		group_by(modelo) %>%
		roc_curve(type, {{ verdad }}, event_level = evto_pred) %>%
...
```{r, roc, fig.cap='(ref:roc)', fig.width=13, fig.asp=0.7}

	# tomar en cuenta que `collect_predictions()` puede sumarizar los diversos
	# resultados sobre las predicciones replicadas fuera de la muestra.
	.datos <- best %>% 
		imap_dfr(~ tune_res %>% 
						extract_workflow_set_result(id = .x) %>% 
						collect_predictions(), .id = "modelo")
...
best %>% 
	imap_dfr(~ tune_res %>% 
					extract_workflow_set_result(id = .x) %>% 
					collect_predictions() %>% 
					select(contains("spam"), type), .id = "modelo") %>% 
	group_by(modelo) %>% 
	roc_curve(type, .pred_spam, event_level = "second") %>% class()
	autoplot() +
	# ggplot(aes(x = 1 - specificity, y = sensitivity, color = modelo)) +
	# geom_line(size = 1, alpha = 0.5) +
	# geom_abline(lty = 2,
...
Con base a la tabla \@ref(tab:modelos) y a la figura \@ref(fig:roc)
seleccionemos los dos mejores modelos de cada motor para probarlos contra el
dataset de prueba.
...
```{r}
(svp <- info_models$smote_simple_svm_p_kernlab)
```
...
- Con una función de costo = `r svp@param$C`
- Tenemos `r svp@nSV` vectores de soporte, 

```{r}
(knd <- info_models$submuestreo_knn)
```

- El mejor $k = 9$, el cual es impar.
...
combinación de receta de preprocesaimiento en conjunto con un motor (modelo) a
...
En esta lista vemos los resultados de los modelos seleccionados con los datos
de validación.
...
test_result_list %>% 
	...
	relocate(specificity, roc_auc, kap, .after = modelo) %>% 
